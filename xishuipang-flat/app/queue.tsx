  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.bgCanvas }}>
      ...
      <ScrollView style={{ flex: 1 }}
        ...
        <Text style={{ fontSize: isMobile ? 26 : 36, fontWeight: '700', letterSpacing: -1, color: theme.textPrimary }}>播放队列</Text>
        <Text style={{ fontSize: fontSize.body, color: theme.textSecondary, marginTop: 8, marginBottom: spacing.xl }}>
          {queue.length} 首 · 用 ↑↓ 调整顺序
        </Text>
        ...
        {queue.length === 0 ? (
          ...
          queue.map((it, i) => (
            <View key={`${it.id}-${i}`} style={{
              ...
              borderWidth: 1, borderColor: i === currentIdx ? theme.brand : theme.borderSoft,
              ...
              <View style={{
                ...
                <Text style={{ color: '#fff', fontSize: isMobile ? 14 : 16, fontWeight: '700' }}>{it.title.slice(0, 1)}</Text>
              </View>
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text numberOfLines={1} style={{ fontSize: isMobile ? fontSize.small : fontSize.body, fontWeight: '600', color: theme.textPrimary }}>{it.title}</Text>
                <Text numberOfLines={1} style={{ fontSize: isMobile ? 11 : fontSize.caption, color: theme.textSecondary, marginTop: 2 }}>
                  {it.author} · {it.mins} 分钟{i === currentIdx ? ' · 正在播放' : ''}
                </Text>
              </View>
...
function QueueBtn({ icon, onPress, disabled, danger }: {
  ...
  return (
    <Pressable onPress={disabled ? undefined : onPress}
      style={({ pressed }) => ({
        ...
        backgroundColor: pressed && !disabled ? (danger ? theme.danger : theme.brandSoft) : 'transparent',
        opacity: disabled ? 0.3 : 1,
      })}>
      <Text style={{ fontSize: 13, color: danger ? theme.danger : theme.textSecondary }}>{icon}</Text>
    </Pressable>
  );
}
