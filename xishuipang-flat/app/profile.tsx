  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.bgCanvas }}>
      ...
      <ScrollView style={{ flex: 1 }}
        ...
        {/* 设置 */}
        <Text style={{ fontSize: 20, fontWeight: '700', color: theme.textPrimary, marginTop: spacing.xl, marginBottom: spacing.md + 2 }}>
          偏好设置
        </Text>
        <Pressable
          ...
          style={{
            ...
            borderRadius: 14, padding: spacing.md + 2, marginBottom: spacing.lg,
          }}>
          <Text style={{ fontSize: fontSize.body, fontWeight: '600', color: theme.textPrimary }}>显示语言</Text>
          <Text style={{ fontSize: fontSize.body, color: theme.brand, fontWeight: '600' }}>
            {character === 'simplified' ? '简体中文 → 切换繁體' : '繁體中文 → 切换简体'}
          </Text>
        </Pressable>

        {/* 收藏 */}
        <View style={{ flexDirection: 'row', alignItems: 'baseline', marginTop: spacing.xl, marginBottom: spacing.md + 2 }}>
          <Text style={{ fontSize: 20, fontWeight: '700', color: theme.textPrimary }}>我喜欢的内容</Text>
          <Pressable onPress={() => router.push('/favorites')}>
            <Text style={{ fontSize: fontSize.small, fontWeight: '600', color: theme.textSecondary, marginLeft: spacing.md }}>管理 →</Text>
          </Pressable>
        </View>
        {favItems.length === 0 ? (
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap }}>
            {favItems.slice(0, 6).map(a => (
              <InfoRow key={a.id} item={a} meta={`${a.author} · 第 ${a.volume} 期`}
                itemWidth={itemWidth}
                onOpen={() => router.push(`/article/${encodeURIComponent(a.id)}`)}
                onRemove={() => toggleFav(a)} />
            ))}
          </View>
        )}
      </ScrollView>
...
function InfoRow({ item, meta, itemWidth, onOpen, onRemove }: {
  item: ContentItem; meta: string; itemWidth: number; onOpen: () => void; onRemove: () => void;
}) {
  const { theme } = useTheme();
  return (
    <View style={{
      width: itemWidth, backgroundColor: theme.bgElevated, borderWidth: 1, borderColor: theme.borderSoft,
      ...
      <Pressable onPress={onOpen} style={{ flex: 1, minWidth: 0 }}>
        ...
        <Text numberOfLines={1} style={{ fontSize: fontSize.caption, color: theme.textSecondary, marginTop: 3 }}>{meta}</Text>
      </Pressable>
