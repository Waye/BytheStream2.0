  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.bgCanvas }}>
      ...
      <ScrollView style={{ flex: 1 }}
        ...
        <Text style={{ fontSize: isMobile ? 26 : 36, fontWeight: '700', letterSpacing: -1, color: theme.textPrimary }}>我喜欢的</Text>
        <Text style={{ fontSize: fontSize.body, color: theme.textSecondary, marginTop: 6, marginBottom: spacing.xl }}>
          {favItems.length} 篇 · 点 ✕ 移除收藏
        </Text>
        ...
        {favItems.length === 0 ? (
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap }}>
            {favItems.map(a => (
              <View key={a.id} style={{
                width: itemWidth, backgroundColor: theme.bgElevated, borderWidth: 1,
                borderColor: theme.borderSoft, borderRadius: 14, padding: spacing.md + 2,
                ...
                <Pressable onPress={() => router.push(`/article/${encodeURIComponent(a.id)}`)} style={{ flex: 1, minWidth: 0 }}>
                  <Text numberOfLines={1} style={{ fontSize: fontSize.body, fontWeight: '700', color: theme.textPrimary }}>{a.title}</Text>
                  <Text numberOfLines={1} style={{ fontSize: fontSize.caption, color: theme.textSecondary, marginTop: 3 }}>
                    {a.author} · 第 {a.volume} 期 · {a.mins} 分钟
                  </Text>
                </Pressable>
                <Pressable onPress={() => toggleFav(a)}
                  style={({ pressed }) => ({
